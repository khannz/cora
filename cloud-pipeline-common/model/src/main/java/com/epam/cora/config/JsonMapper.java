...
public class JsonMapper extends ObjectMapper {
    ...
    public static <T> String convertDataToJsonStringForQuery(T data) {
        return convertDataToJsonStringForQuery(data, instance);
    }

    public static <T> String convertDataToJsonStringForQuery(T data, ObjectMapper customMapper) {
        if (data == null) {
            return EMPTY_JSON;
        }
        String resultData;
        try {
            resultData = customMapper.writeValueAsString(data);
    ...
    public static <T> T parseData(final String data,
        ...
        try {
            return customMapper.readValue(data, type); // FIXME
        } catch (IOException e) {
            throw new IllegalArgumentException("Could not parse json data " + data, e);
        }
    }

    public static boolean isNullOrAnyValidJson(final String data) {
        if (StringUtils.isBlank(data)) {
            return true;
        }
    ...
    public static void writeData(final Object data, final File file) {
        writeData(data, file, instance);
    }

    public static void writeData(final Object data, final File file, final ObjectMapper customMapper) {
        try {
            customMapper.writeValue(file, data);
        } catch (IOException e) {
            throw new IllegalArgumentException("Could not write data to json file " + file.toString(), e);
        }
    }
