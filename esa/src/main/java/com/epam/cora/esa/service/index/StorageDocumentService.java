package com.epam.cora.esa.service.index;
...
import com.epam.cora.esa.service.impl.IndexRequestContainer;
...
public class StorageDocumentService {
    ...
    public void indexFile(final Long storageId, final List<DataStorageFile> files) {
        final AbstractDataStorage storage = cloudPipelineAPIClient.loadDataStorage(storageId);
        final String region = getRegion(storage);

        final EntityPermissionVO entityPermission = cloudPipelineAPIClient.loadPermissionsForEntity(storage.getId(), storage.getAclClass());

        final PermissionsContainer permissionsContainer = new PermissionsContainer();
        permissionsContainer.add(Optional.ofNullable(entityPermission).map(EntityPermissionVO::getPermissions).orElse(Collections.emptySet()), storage.getOwner());
        ...
        try {
            ...
            log.debug("Inserting {} document(s) for storage {} into indices {}", files.size(), storageId, indices);

            indices.forEach(index -> {
                try (IndexRequestContainer requestContainer = getRequestContainer(index, bulkInsertSize)) {
                    files.stream().map(file -> createIndexRequest(file, storage, permissionsContainer, index, region)).forEach(requestContainer::add);
                }
            });
        } finally {
            if (simpleLock != null) {
    ...
    private IndexRequest createIndexRequest(final DataStorageFile file, final AbstractDataStorage dataStorage, final PermissionsContainer permissionsContainer, final String indexName, final String region) {
        return new IndexRequest(indexName, DOC_MAPPING_TYPE, file.getPath()).source(mapper.fileToDocument(file, dataStorage, region, permissionsContainer, getDocumentType(dataStorage), tagDelimiter, null));
    }
