...
import com.epam.cora.esa.service.impl.converter.pipeline.PipelineIdConverter;
import com.epam.cora.esa.service.impl.converter.pipeline.PipelineLoader;
import com.epam.cora.esa.service.impl.converter.pipeline.PipelineMapper;
...
public class PipelineSynchronizer implements ElasticsearchSynchronizer {
    ...
    public PipelineSynchronizer(final PipelineEventDao pipelineEventDao, final @Value("${sync.pipeline.index.mapping}") String pipelineIndexMappingFile, final @Value("${sync.pipeline-code.index.mapping}") String pipelineCodeIndexMappingFile, final @Value("${sync.index.common.prefix}") String indexPrefix, final @Value("${sync.pipeline.index.name}") String pipelineIndexName, final @Value("${sync.pipeline-code.index.name}") String pipelineCodeIndexName, final @Value("${sync.pipeline-code.index.paths}") String pipelineFileIndexPaths, final @Value("${sync.pipeline-code.bulk.insert.size}") Integer bulkInsertSize, final CloudPipelineAPIClient cloudPipelineAPIClient, final ElasticsearchServiceClient elasticsearchServiceClient, final ElasticIndexService indexService, final PipelineLoader loader, final PipelineMapper mapper, final PipelineCodeHandler codeHandler, final BulkResponsePostProcessor bulkResponsePostProcessor) {
        ...
        PipelineIdConverter idConverter = new PipelineIdConverter(indexPrefix + pipelineIndexName, indexPrefix + pipelineCodeIndexName);
        this.requestSender = new BulkRequestSender(elasticsearchClient, bulkResponsePostProcessor, idConverter, bulkInsertSize);
    }
    ...
    public void synchronize(final LocalDateTime lastSyncTime, final LocalDateTime syncStart) {
        ...
        final List<PipelineEvent> pipelineEvents = EventProcessorUtils.mergeEvents(pipelineEventDao.loadPipelineEventsByObjectType(PipelineEvent.ObjectType.PIPELINE, syncStart));

        final List<PipelineEvent> pipelineCodeEvents = pipelineEventDao.loadPipelineEventsByObjectType(PipelineEvent.ObjectType.PIPELINE_CODE, syncStart);

        if (CollectionUtils.isEmpty(pipelineEvents) && CollectionUtils.isEmpty(pipelineCodeEvents)) {
            log.debug("{} and {} entities for synchronization were not found.", PipelineEvent.ObjectType.PIPELINE, PipelineEvent.ObjectType.PIPELINE_CODE);
            return;
        }

        Stream.of(pipelineEvents, pipelineCodeEvents).flatMap(Collection::stream).collect(Collectors.groupingBy(PipelineEvent::getObjectId)).forEach((id, events) -> synchronizePipelineEvents(id, events, syncStart));

        log.debug("Successfully finished {} and {} synchronization.", PipelineEvent.ObjectType.PIPELINE, PipelineEvent.ObjectType.PIPELINE_CODE);
    }

    @SuppressWarnings("PMD.AvoidCatchingGenericException")
    private void synchronizePipelineEvents(final Long pipelineId, final List<PipelineEvent> events, final LocalDateTime syncStart) {
        try {
            final String indexNameForPipeline = indexPrefix + pipelineIndexName;
            final String indexNameForPipelineCode = String.format("%s%s-%d", indexPrefix, pipelineCodeIndexName, pipelineId);

            final PipelineDocRequests pipelineRequests = buildDocRequests(pipelineId, events, indexNameForPipeline, indexNameForPipelineCode);
            ...
            if (CollectionUtils.isNotEmpty(pipelineRequests.getPipelineRequests())) {
                requestSender.indexDocuments(indexNameForPipeline, Arrays.asList(PipelineEvent.ObjectType.PIPELINE, PipelineEvent.ObjectType.PIPELINE_CODE), pipelineRequests.getPipelineRequests(), syncStart);
            }
            if (CollectionUtils.isNotEmpty(pipelineRequests.getCodeRequests())) {
                ...
                requestSender.indexDocuments(indexNameForPipelineCode, PipelineEvent.ObjectType.PIPELINE_CODE, pipelineRequests.getCodeRequests(), syncStart);
            }
        } catch (Exception e) {
    ...
    private PipelineDocRequests buildDocRequests(final Long pipelineId, final List<PipelineEvent> events, final String pipelineIndex, final String codeIndex) {
        PipelineDocRequests.PipelineDocRequestsBuilder requestsBuilder = PipelineDocRequests.builder().pipelineId(pipelineId);
        try {
            log.debug("Processing pipeline {} events {}", pipelineId, events);
            final List<PipelineEvent> pipelineEvents = ListUtils.emptyIfNull(events).stream().filter(event -> event.getObjectType() == PipelineEvent.ObjectType.PIPELINE).collect(Collectors.toList());
            ...
            if (CollectionUtils.isEmpty(pipelineEvents)) {
                return requestsBuilder.codeRequests(pipelineCodeHandler.processGitEvents(pipelineId, events)).build();
            } else {
    ...
    private PipelineDocRequests processPipelineEvent(final PipelineEvent event, final String pipelineIndex, final String codeIndex) throws EntityNotFoundException {
        PipelineDocRequests.PipelineDocRequestsBuilder requestsBuilder = PipelineDocRequests.builder().pipelineId(event.getObjectId());
        ...
        loader.loadEntity(event.getObjectId()).ifPresent(pipelineEntity -> {
    ...
    private PipelineDocRequests cleanCodeIndexAndCreateDeleteRequest(final Long pipelineId, final String pipelineIndex, final String codeIndex, final PipelineDocRequests.PipelineDocRequestsBuilder requestsBuilder) {
        elasticsearchClient.deleteIndex(codeIndex);
        requestsBuilder.pipelineRequests(Collections.singletonList(new DeleteRequest(pipelineIndex, INDEX_TYPE, String.valueOf(pipelineId))));
        return requestsBuilder.build();
    }
