...
import com.epam.cora.entity.pipeline.PipelineTask;
...
public class PipelineRunMapper implements EntityMapper<PipelineRunWithLog> {
    ...
    private String getRunDescription(final PipelineRun pipelineRun) {
        if (StringUtils.isNotBlank(pipelineRun.getPipelineName()) && StringUtils.isNotBlank(pipelineRun.getVersion())) {
    ...
    private void buildRunInstance(RunInstance instance, XContentBuilder jsonBuilder) throws IOException {
        ...
        jsonBuilder.field("instance").startObject().field("nodeType", instance.getNodeType()).field("nodeDisk", instance.getNodeDisk()).field("nodeIP", instance.getNodeIP()).field("nodeId", instance.getNodeId()).field("nodeImage", instance.getNodeImage()).field("nodeName", instance.getNodeName()).field("priceType", instance.getSpot()).field("cloudRegionId", instance.getCloudRegionId()).endObject();
    }
    ...
    private void buildRunParam(List<PipelineRunParameter> runParams, XContentBuilder jsonBuilder) throws IOException {
        ...
        jsonBuilder.array("parameters", runParams.stream().map(param -> {
            final String paramName = param.getName();
            final String paramValue = StringUtils.defaultIfBlank(StringUtils.defaultIfBlank(param.getResolvedValue(), param.getValue()), StringUtils.EMPTY);
    ...
    private void buildRunLog(List<RunLog> runLogs, XContentBuilder jsonBuilder) throws IOException {
        ...
        jsonBuilder.array("logs", runLogs.stream().sorted(Comparator.comparing(RunLog::getDate).reversed()).limit(maxLogLines).map(log -> {
            final String logText = log.getLogText();
            final String taskName = Optional.ofNullable(log.getTask()).map(PipelineTask::getName).orElse(StringUtils.EMPTY);
