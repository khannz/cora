...
public class PipelineEventDao extends NamedParameterJdbcDaoSupport {
    ...
    @Transactional(propagation = Propagation.SUPPORTS)
    public List<PipelineEvent> loadPipelineEventsByObjectType(final PipelineEvent.ObjectType objectType, final LocalDateTime before, final Integer rowLimit) {
        ...
        final List<PipelineEvent> pipelineEvents = getNamedParameterJdbcTemplate().query(loadAllEventsByObjectTypeQuery, parameterSource, PipelineEventsParameters.getRowMapper());
        return pipelineEvents;
    }

    @Transactional(propagation = Propagation.REQUIRED)
    public void deleteEventByObjectId(final Long id, final PipelineEvent.ObjectType objectType, final LocalDateTime before) {
    ...
    enum PipelineEventsParameters {
        OPERATION, STAMP, OBJECT_TYPE, OBJECT_ID, DATA, LIMIT;
