...
import com.epam.cora.entity.datastorage.AbstractDataStorageItem;
...
public class StorageFileMapper {
    ...
    public XContentBuilder fileToDocument(final DataStorageFile dataStorageFile, final AbstractDataStorage dataStorage, final String region, final PermissionsContainer permissions, final SearchDocumentType type, final String tagDelimiter, final String content) {
        try (XContentBuilder jsonBuilder = XContentFactory.jsonBuilder()) {
            ...
            jsonBuilder.startObject().field("lastModified", dataStorageFile.getChanged()).field("size", dataStorageFile.getSize()).field("path", dataStorageFile.getPath()).field("cloud_path", constructCloudPath(dataStorage, dataStorageFile)).field("mount_path", constructMountPath(dataStorage, dataStorageFile)).field("id", dataStorageFile.getPath()).field("name", dataStorageFile.getName()).field("ownerUserName", tags.get("CP_OWNER")).field("parentId", dataStorage.getId()).field("storage_id", dataStorage.getId()).field("storage_name", dataStorage.getName()).field("storage_region", region).field("is_hidden", isHidden(dataStorage, dataStorageFile)).field("is_deleted", Boolean.TRUE.equals(dataStorageFile.getDeleteMarker())).field(DOC_TYPE_FIELD, type.name()).array("metadata", tags.entrySet().stream().map(entry -> entry.getKey() + " " + entry.getValue()).toArray(String[]::new)).array("allowed_users", permissions.getAllowedUsers().toArray()).array("denied_users", permissions.getDeniedUsers().toArray()).array("allowed_groups", permissions.getAllowedGroups().toArray()).array("denied_groups", permissions.getDeniedGroups().toArray()).field("content", content).field("storage_class", labels.getOrDefault(ESConstants.STORAGE_CLASS_LABEL, STANDARD_TIER));

            if (MapUtils.isNotEmpty(dataStorageFile.getVersions())) {
                final Map<String, ImmutablePair<Long, Integer>> versionSizesByStorageClass = calculateVersionSizes(dataStorageFile.getVersions());
                for (String key : versionSizesByStorageClass.keySet()) {
                    String storageClassKey = key.toLowerCase(Locale.ROOT);
                    jsonBuilder.field(String.format("ov_%s_size", storageClassKey), versionSizesByStorageClass.get(key).getLeft());
                    jsonBuilder.field(String.format("ov_%s_count", storageClassKey), versionSizesByStorageClass.get(key).getRight());
                }
            }

            for (final Map.Entry<String, String> entry : tags.entrySet()) {
                if (StringUtils.hasText(tagDelimiter) && StringUtils.hasText(entry.getValue()) && entry.getValue().contains(tagDelimiter)) {
    ...
    private Map<String, ImmutablePair<Long, Integer>> calculateVersionSizes(final Map<String, AbstractDataStorageItem> versions) {
        ...
        for (final AbstractDataStorageItem v : versions.values()) {
            ...
            final String storageClass = Optional.ofNullable(MapUtils.emptyIfNull(version.getLabels()).get(ESConstants.STORAGE_CLASS_LABEL)).orElse(STANDARD_TIER);
            result.compute(storageClass, (tier, tierSize) -> {
                if (tierSize == null) {
    ...
    private static String constructMountPath(AbstractDataStorage dataStorage, DataStorageFile dataStorageFile) {
        ...
        if (StringUtils.isEmpty(mountPoint)) {
            final String storageMountName = DataStorageType.NFS == dataStorage.getType() ? dataStorage.getPath().replaceFirst(":", "") : dataStorage.getPath();
    ...
    public void updateSearchMasks(final CloudPipelineAPIClient cloudPipelineAPIClient, final Logger logger) {
        ...
        final Map<String, Set<String>> newHiddenMasks = getSearchMasks(storageSearchMasks, StorageFileSearchMask::getHiddenFilePathGlobs);
        hiddenMasks.clear();
        logger.info("Updating search hidden masks: {}", newHiddenMasks);
        hiddenMasks.putAll(newHiddenMasks);

        final Map<String, Set<String>> newIndexContentMasks = getSearchMasks(storageSearchMasks, StorageFileSearchMask::getIndexedContentPathGlobs);
    ...
    private Map<String, Set<String>> getSearchMasks(final List<StorageFileSearchMask> storageSearchMasks, final Function<StorageFileSearchMask, Set<String>> getMask) {
        return ListUtils.emptyIfNull(storageSearchMasks).stream().filter(mask -> CollectionUtils.isNotEmpty(getMask.apply(mask))).collect(Collectors.toMap(StorageFileSearchMask::getStorageName, getMask, SetUtils::union));
    }
    ...
    private boolean isMaskMatch(final String storageName, final String filePath, final Map<String, Set<String>> masks) {
        if (masks.containsKey(storageName)) {
            ...
            return CollectionUtils.emptyIfNull(masks.get(storageName)).stream().anyMatch(mask -> pathMatcher.match(mask, filePath));
        }
        return false;
    }
}
